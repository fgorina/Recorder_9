...
- (void) setDefaultTimeZone:(NSTimeZone *)tz {
    if (_defaultTimeZone != tz) {
        _defaultTimeZone = nil;
        _defaultTimeZone = tz;
        
        self.unparsingCalendar.timeZone = _defaultTimeZone;
    }
}
...
- (NSDateComponents *) dateComponentsFromString:(NSString *)string timeZone:(out NSTimeZone **)outTimeZone range:(out NSRange *)outRange {
    ...
    NSAssert(timeSep != '\0', NSLocalizedString(@"Time separator must not be NUL." ,@"Separador de temps incorrecte"));
...
- (NSString *) stringFromDate:(NSDate *)date timeZone:(NSTimeZone *)timeZone {
    switch (self.format) {
        ...
        default:
            [NSException raise:NSInternalInconsistencyException format:@"self.format was %ld, not calendar (%ld), week (%ld), or ordinal (%ld)", (unsigned long)self.format,(unsigned long) ISO8601DateFormatCalendar, (unsigned long)ISO8601DateFormatWeek, (unsigned long)ISO8601DateFormatOrdinal];
            return nil;
    }
}
...
- (NSString *) stringFromDate:(NSDate *)date formatString:(NSString *)dateFormat timeZone:(NSTimeZone *)timeZone {
    ...
    if (!unparsingFormatter) {
        ...
        unparsingFormatter.calendar = self.unparsingCalendar;
    }
